export default (S) =>
  S.list()
    ...
    .items([
      ...
      // Resources no longer used. Commenting out for potential future use
      // S.listItem()
      //     .title('Resources')
      //     .child(
      //         S.document()
      ...
      S.listItem()
        .title("Assets")
        .child(S.documentList().title("Assets").filter('_type == "assets"')),
      ...S.documentTypeListItems().filter((listItem) =>
        ["location"].includes(listItem.getId())
      ),
      S.listItem()
        .title("Pop Ups")
        .child(S.document().schemaType("active_pop_up").documentId("popUps")),
      S.listItem()
        .title("FAQs")
        .child(S.document().schemaType("faqs").documentId("faQs")),
      S.listItem()
        .title("Login Link")
        .child(S.document().schemaType("login").documentId("loginLink")),
    ]);
